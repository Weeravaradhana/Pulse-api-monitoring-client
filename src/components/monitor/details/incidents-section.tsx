...
function formatDate(iso: string) {
    if (!iso) return "N/A"; // Date string එකක් නැතිව ආවොත් crash වීම වලක්වයි
...
function IncidentCard({ incident }: IncidentCardProps) {
    // 💡 CRITICAL BUG FIX: incident.severity එකේ mapping එකක් නැති වුනොත් (उदा. unknown status) UI crash නොවී බේරගන්න fallback එකක් දැම්මා
    const cfg      = SEVERITY_CONFIG[incident.severity] || SEVERITY_CONFIG.info;
    const SevIcon  = cfg.icon;
    const resolved = incident.resolvedAt !== null;

    return (
        <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 border-l-4 ${cfg.border} rounded-xl p-4 transition-colors`}>
            <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        {/* UI එක වෙනස් නොකර ඔයාගේ original class එකම තබා ඇත */}
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-200 wrap-break-word">
                            {incident.cause}
                        </p>
                    </div>
                </div>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                    ...
                    { label: "Duration",       value: incident.duration || "N/A" },
